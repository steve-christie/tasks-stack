...
export const taskReducer = (state: ITasksState = initialState, action: IAction<boolean| string| ITask | ITask[]>) => {
    switch(action.type) {
        ...
        case CREATE_TASK[REQUEST]: return {
            ...state,
            creatingTask: true,
        }
        ...
        case CREATE_TASK[FAILURE]: return {
            ...state,
            creatingTask: false,
        ...
        case DELETE_TASK[REQUEST]: {
            ...
            const taskToDelete = state.taskStates.find(ts => ts.task.taskId === action.payload)
        ...
        case DELETE_TASK[SUCCESS]: {
            const taskIndex = state.taskStates.findIndex(ts => ts.task.taskId === action.payload)
        ...
        case UPDATE_TASK[REQUEST]: {
            ...
            const taskToUpdate = state.taskStates.find(ts => ts.task.taskId === taskUpdatePayload.taskId)
        ...
        case UPDATE_TASK[SUCCESS]: {
            ...
            const updatedTaskIndex = state.taskStates.findIndex(ts => ts.task.taskId === updatedTask.taskId)
