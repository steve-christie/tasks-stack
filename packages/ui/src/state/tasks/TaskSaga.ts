...
export function* createNewTaskSaga(action: IAction<Partial<ITask>>) {
  try {
    ...
    yield put(
      taskActions.createTask.failure("An error occurred creating task")
    );
  }
}
...
export function* deleteTaskSaga(action: IAction<string>) {
  try {
    ...
    yield put(
      taskActions.deleteTask.failure("An error occurred deleting task")
    );
  }
}
...
export function* fetchTasksSaga(action: IAction<IFetchTasksFilters>) {
  try {
    const taskResults: ITask[] = yield call(
...
export function* updateTaskSaga(action: IAction<Partial<ITask>>) {
  try {
    const updatedTask: ITask = yield call(
    ...
    yield put(
      taskActions.updateTask.failure("An error occurred updating task")
    );
  }
}
